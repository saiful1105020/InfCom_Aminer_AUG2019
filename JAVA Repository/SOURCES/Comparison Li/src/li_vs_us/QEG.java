...
import static li_vs_us.Main.authors;
...
public class QEG {
    ...
    public QEG(KICQ kicq, boolean noDecomposition) {
        int n;
        n = kicq.keywords.length;
    ...
    public QEG(KICQ kicq, Set<Integer> vertexSet) {
        ...
        */
        ...
        //System.out.println(this.V);
        for (int nodeId : this.V) {
            Node node = new Node(TreeExplore.qeg.idNodeMap.get(nodeId));
            node.adjList.retainAll(this.V);
    ...
    //connected component in vSet that contains vertexId
    public Set<Integer> findConnectedComponent(Set<Integer> vSet, int vertexId) {
        HashMap<Integer, Boolean> visited = new HashMap<>();
        ...
        Set<Integer> C = new LinkedHashSet<>();
        bfsVisit(C, vertexId, vSet, visited);
