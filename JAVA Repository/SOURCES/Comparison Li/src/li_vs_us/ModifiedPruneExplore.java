...
public class ModifiedPruneExplore {
    ...
    Set<Integer> exclusiveVertices;

    public ModifiedPruneExplore(QEG qeg, Set<Integer> exclusiveVertices, int k_min, int k_max, PriorityQueue<Community> Q) {
        this.qeg = qeg;
        this.Q = Q;
        this.exclusiveVertices = exclusiveVertices;
    ...
    public void solve(Set<Integer> H, int k, int k_max) {
        ...
        */
        ...
        for (int i = 0; i < components.size(); i++) {
            ...
            Set<Integer> exclusiveComponentVerts = new HashSet<Integer>(this.exclusiveVertices);
            exclusiveComponentVerts.retainAll(componentNodes);
            if(exclusiveComponentVerts.size()==0)
            {
                continue;
            }
