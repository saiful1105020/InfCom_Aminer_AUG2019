...
#print(json.dumps(keywordMap))

fw.write(json.dumps(keywordMap))
fw.flush()

f.close()
fw.close()
print("Keyword ID Map written into file")
...
for dir_index in range(0, 3):
    ...
    OUT_DIR = "../../Dataset_Preprocessed/authID_keywordID" + str(dir_index) + "/"
    ...
    for fid in range(global_index, global_index + total_files[dir_index]):
        ...
        writefilename = OUT_DIR+"summary_all_id_"+str(fid)+".txt"
        # data file
        f = open(filename, encoding='utf-8', mode='r')
        # file containing ids of relevant keywords
...
#Convert input term into set of relevant keywords
