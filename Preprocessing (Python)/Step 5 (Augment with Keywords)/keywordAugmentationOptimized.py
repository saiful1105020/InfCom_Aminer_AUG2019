...
print("Stage 2: Word Vector Loaded")
...
def getSimilarVector(keyword):
    ...
    similar_words = []
    try:
        similar_words = model.wv.most_similar(positive = lemmatized_words, topn=TOP_N)
    except:
        #The given keyword is not found in the dictionary
        print("Exception in Similar Words Method")
        pass
...
def writeFile(writefilename, outFileContents):
    print("Write File Started")
    fw = open(writefilename, encoding='utf-8', mode='w')
    for jsonItem in outFileContents:
        fw.write(jsonItem)
        fw.write("\n")
        fw.flush()
    fw.close()
...
if __name__ == '__main__':
    ...
    for keyword in keywords:
        keyword_topNWords[keywordMap[keyword]] = getSimilarVector(keyword)
    ...
    writefilename = OUT_DIR+"augmented_"+str(file_index)+".txt"
    ...
    for s in fileContents:
        ...
        if count % 500 == 0:
            ...
            print('Duration: {} seconds'.format(time.time() - starttime))
            starttime = time.time()
            break
        ...
        for x in d["keywords"]:
            try:
                ...
                if input_term in similarKeywordsMap:
                    ...
                    input_topNWords = getSimilarVector(input_term)

                    similarity = {}
                    for kid in range(1, num_keywords + 1):
                        similarity[kid] = nTopSimilarity(input_topNWords, keyword_topNWords[kid])
        ...
        outFileContents.append(json_data)

    p = Process(target=writeFile, args=(writefilename,outFileContents))
    p.start()
    processes.append(p)

    program_end_time = time.time()
    print('Program Runtime: {} seconds'.format(program_end_time - program_start_time))
    for p in processes:
        p.join()
    exit(0)