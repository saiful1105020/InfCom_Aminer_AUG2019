...
def nTopSimilarity(X_vec, Y_vec):
    ...
    f_timestamp = time.time_ns()
    print("One sim calculation: {} nano seconds".format(f_timestamp-i_timestamp))
    exit(0)
    return result
    ...
    while file_index<=154:
        ...
        for s in data_items:
            ...
            if count%500==0:
                ...
                print(("Processing: File#"+str(file_index)+" >> Article: "+str(count)+" \t {} seconds").format(current_time-init_time))
            ...
            for x in d["keywords"]:
                ...
                try:
                    ...
                    if input_term in similarKeywordsMap:
                        sim_key_ids = similarKeywordsMap[input_term]
                    else:
                        timestamp = time.time_ns()
                        input_topNWords = getSimilarVector(input_term, k=TOP_N)
                        print("Get similar vector: \t {} nanoseconds".format(time.time_ns() - timestamp))

                        print("Keyword size: "+str(len(keywords)))

                        similarity = {}
                        timestamp = time.time_ns()
                        for y in keywords:
                            similarity[y] = nTopSimilarity(input_topNWords, keyword_topNWords[y])
                        print("Similarity Calculation: \t {} nanoseconds".format(time.time_ns() - timestamp))
                        ...
                        if numpy.math.isnan(kv[1]) else kv[1], reverse=True)[0:topK]:
                            if (item[1] > mapping_threshold):
                                ...
                                break
                        similarKeywordsMap[input_term] = sim_key_ids
                        print("Top k finding: \t {} seconds".format(time.time_ns() - timestamp))
                    for key_id in sim_key_ids:
                        if key_id not in similar_keywords:
                            similar_keywords.append(key_id)
                except Exception as e:
                    print(e)
            ...
            exit(0)
        #process = multiprocessing.Process(target=writeFile,args=(writefilename,results))
        #process.start()

        #process.join()
        writeFile(writefilename,results)
        file_index = file_index + num_processes
    print("Finished")
